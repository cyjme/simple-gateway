...
func (n *node) addRoute(path string, handle string) {
	fullPath := path
	n.priority++
...
func (n *node) insertChild(numParams uint8, path, fullPath string, handle string) {
...
// given path.
func (n *node) getValue(path string) (handle string, p Params, tsr bool) {
	...
	for {
		...
		tsr = (path == "/") ||
			(len(n.path) == len(path)+1 && n.path[len(path)] == '/' &&
				path == n.path[:len(n.path)-1] && n.handle != "")
		return
	}
}
...
walk: // outer loop for walking the tree
	for len(loPath) >= len(loNPath) && (len(loNPath) == 0 || loPath[1:len(loNPath)] == loNPath[1:]) {
		...
		if path = path[len(n.path):]; len(path) > 0 {
			...
			if !n.wildChild {
				...
				return ciPath, (fixTrailingSlash && path == "/" && n.handle != "")
			}
	...
	if fixTrailingSlash {
		...
		if len(loPath)+1 == len(loNPath) && loNPath[len(loPath)] == '/' &&
			loPath[1:] == loNPath[1:len(loPath)] && n.handle != "" {
